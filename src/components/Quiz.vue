    ...
    <div style="margin-bottom: 64px">
        ...
        <div v-for="(item, index) in quizItems" :key=index>
            ...
            <div class="answer-box" v-for="(answer, aIndex) in item.answers" :key=aIndex>
                <input type="radio" :name="item.issue" value="maison"><span class="answer-span">{{ answer }}</span><br>
            </div>
        </div>
        <button style="font-size: 32px">See Results</button>
    </div>
</template>
<script>
export default {
